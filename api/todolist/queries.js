...
const getTodos = (req, res) => {
	pool.query(`SELECT * FROM ${todoListTable} ORDER BY id ASC`, (err, result) => {
		if (err) {
			res.status(500).json({
				err: '500 Database Down Error'
			});
			return;
		}
...
const getTodoById = (req, res) => {
	...
	if (Number.isNaN(id)) {
		...
		pool.query(`SELECT * FROM ${todoListTable} WHERE id= $2`, [ id ], (err, result) => {
			if (err) {
				res.status(500).json({
					err: 'Database Error'
				});
...
const deleteTodo = (req, res) => {
	...
	if (Number.isNaN(id)) {
		...
		pool.query(`DELETE FROM ${todoListTable} WHERE id=$1`, [ id ], (err, result) => {
			if (err) {
				res.status(500).json({
					err: '500 Database Error'
				});
...
const toggleTodo = (req, res) => {
	...
	if (Number.isNaN(id)) {
		...
		pool.query(`UPDATE ${todoListTable} SET complete= NOT complete WHERE ID = $1`, [ id ], (err, result) => {
			if (err) {
				res.status(500).json({
					err: '500 Database Error'
				});
